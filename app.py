...
st.markdown("""
    ...
    /* Quitar espacios blancos gigantes (padding) */
    .block-container {
...
# --- 3. LÓGICA DE CHAT ---
# Título limpio sin imágenes rotas
...
if prompt := st.chat_input("Escribe tu consulta técnica aquí..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analizando base de datos de ingeniería..."):
            try:
                # CONEXIÓN DB (Solo se activa si hay secretos, sino usa modo demo)
                if "SUPABASE_URL" in st.secrets:
                    ...
                    # Modo seguro si fallan las llaves
                    full_response = "Modo Demo: No puedo acceder a la base de datos ahora mismo, pero soy el Asistente Semhys."
                
                st.markdown(full_response)
                st.session_state.messages.append({"role": "assistant", "content": full_response})
            except Exception as e:
                err_msg = "Lo siento, hubo un error de conexión con el servidor de IA."
                st.error(err_msg)
